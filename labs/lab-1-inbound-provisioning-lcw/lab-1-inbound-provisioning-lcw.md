...
Click to select "API-driven provisioning to Microsoft Entra ID", give it a descriptive name like "ELUK26 API-driven provisioning to Entra ID" and click Create.
...
Click on "Show advanced options", and the click "Edit attribute list for API". Here you can add or change the list of API attributes. Scroll down, and there is an empty name box. You can define your own Schema, so lets add the 2 following attributes (add as strings even though they are dates), change yourorgnamehere to your companyname, or use eluk26 or something similar:
...
2. Give the App a name like for example "API-driven provisioning to Microsoft Entra ID - ELUK Client"
...
Now the user has been created let's create a pre-hire workflow in lifecycle workflows and scope the pre-hire workflow to be executed for users with the department 'ELUK 2026' 7 days prior to the employeeHireDate. For this you need to have, at a bear mnimimum, the Lifecycle Workflow Administrator role assigned. Within this workflow make sure the following actions are set:
...
After the pre-hire workflow has been created, create a new-hire workflow which is triggered based on the employeeHireDate and scoped to users with the departmet 'ELUK 2026'. Within this workflow make sure the following tasks are set:

- Enable Account
- Send Welcome email (feel free to customize on your own)
- Make the user a member of a security group for ELUK 2026
- Make the user a member of a MS Teams group for ELUK 2026
...
At last, create a post-onboarding workflow which is scoped to users with the department 'ELUK 2026' 7 days after the employeeHireDate. Within this workflow make sure the following tasks are executed:
